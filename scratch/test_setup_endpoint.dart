...
void main() async {
  ...
  var response = await client.get(Uri.parse('https://foodchart.free.nf/setup_db.php'), headers: headers);
  if (response.body.contains('slowAES.decrypt') && response.body.contains('toNumbers(')) {
    ...
    if (matches.length >= 3) {
      ...
      response = await client.get(Uri.parse('https://foodchart.free.nf/setup_db.php'), headers: headers);
    }
  }
  print('Status: ${response.statusCode}');
  print('Contains setup: ${response.body.contains("Database Setup")}');
  print('Body snippet: ${response.body.substring(0, response.body.length > 300 ? 300 : response.body.length)}');
}
